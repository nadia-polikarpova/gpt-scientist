...
Можно передавать любые параметры, поддерживаемые OpenAI API:
- `top_p`: Контролирует разнообразие ответов (0.0–1.0). Низкие значения делают ответы более предсказуемыми.
- `max_completion_tokens`: Ограничивает длину ответа. Защищает от чрезмерных расходов при длинных ответах модели.
- `temperature`: Контролирует случайность (0.0–2.0).
- `reasoning_effort`: Для моделей с рассуждениями ('none', 'low', 'medium', 'high').
